...
- [x] T015 · EDHREC client: fetch theme/synergy data for a card — M *(`edhrec.py`, calls `https://json.edhrec.com/pages/cards/{slug}.json` directly via `requests` — chose this over `pyedhrec`/`mightstone`; see DESIGN.md Decision Log for why. Wired straight into `main.py` after the Mechanics print block, same pattern T009 used for Scryfall: `EdhrecNotFoundError` raised uncaught for now — T016's job. Manual smoke test against Lightning Bolt confirmed real similar-cards and synergy-cardlist data print correctly; also confirmed `EdhrecNotFoundError` raises for a nonexistent card name. Manual review found no explicit theme/archetype field anywhere in EDHREC's per-card data — checked both `pages/cards/{slug}.json` and the alternate `cards/{slug}` endpoint, the latter has a sparse `tags` field but it's the same granularity as Scryfall oracle tags (e.g. `["removal"]`, `["ramp"]`, often `None`) and redundant with the Mechanics pass. EDHREC's theme taxonomy lives at the theme-page level, not reverse-linked to individual cards, so the co-occurring-cards synergy signal (commanders/cards this card is commonly played alongside) is the real weak signal here — theme/archetype inference from it is deferred to the LLM pass (T017/T018). Trimmed `main.py`'s printed cardlists to `topcommanders`/`topcards`/`gamechangers` only (was printing all 16, several capped at 50 cards each — way too noisy for manual review); `fetch_edhrec_signal()` itself still returns the full set for T017 to draw from.)*
...
- [x] T016 · Graceful degradation on EDHREC failure — S · depends on T015 *(`main.py` now catches `(EdhrecNotFoundError, requests.RequestException)` around the `fetch_edhrec_signal()` call — narrow scope chosen over a bare `except Exception` so a real bug in our own parsing code wouldn't get silently swallowed as a skip; covers not-found (403/404), network errors, timeouts, and exhausted-retry HTTP errors since `HTTPError` subclasses `RequestException`. On catch: prints `EDHREC: unavailable (...) — skipping weak signal` to stderr and `EDHREC: (skipped)` to stdout, run proceeds normally otherwise. Manually verified both real failure modes: a real Scryfall card EDHREC doesn't index (Un-set card "Squirrel Dealer") and a real network failure (unreachable host) — both completed end-to-end with Mechanics/oracle text printing normally.)*
...
- [x] T017 · Assemble the archetype-classification prompt (oracle text + mechanic tags/confidence + EDHREC signal + archetype list + mechanic-affinity heuristics) — M *(new `archetype.py`: `build_archetype_prompt()`, pure string assembly, no LLM call — that's T018. Five `## `-headed sections in DESIGN.md's exact order: Oracle Text; Mechanic Tags & Confidence (matched mechanic + its taxonomy-doc Confidence label + source oracle tag(s)); EDHREC Signal (full `cardlists`, not main.py's trimmed print-only subset — `(unavailable)` when the fetch was skipped); Candidate Archetypes (the full 25-row Archetypes table, unfiltered — this is the LLM's label space); Mechanic-Archetype Heuristics (the Mechanics/Effects table's Linked Archetype column, unfiltered, rows with no linked archetype dropped). `taxonomy.py` gained three parsers for the two previously-unparsed tables: `parse_archetypes()`, `parse_mechanic_linked_archetypes()`, `parse_mechanic_confidence()`. Finding: the Mechanics/Effects table names one row "Counter", the Oracle Tag Mapping table names the same row "Counter (counterspell)" — the only naming mismatch between the two tables. Rather than special-casing it, the heuristics section is a full static dump of the Linked Archetype column instead of a lookup keyed by this card's matched-mechanic names, which sidesteps the mismatch entirely and matches "embed the column verbatim" more literally anyway; a regression test guards this assumption against a future doc edit "fixing" the naming inconsistency. main.py wired to print the assembled prompt now, ahead of T018 adding the actual LLM call. Manually verified against Lightning Bolt (all five sections populated correctly, in order) and Squirrel Dealer, an Un-set card EDHREC doesn't index (EDHREC section correctly renders `(unavailable)`, rest of the prompt unaffected).)*
